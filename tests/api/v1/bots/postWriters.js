...
/**
 * tests/api/v1/bots/postWriters.js
 */
'use strict';
...
describe('tests/api/v1/bots/postWriters.js >', () => {
  let token;
  let coll;
  let firstUser;
  let secondUser;
  ...
  'associated with the right object', (done) => {
    ...
    .expect((res) => {
      ...
      expect(userOne.botId).to.not.equal(undefined);
      expect(userOne.UserId).to.not.equal(undefined);
      expect(userTwo.botId).to.not.equal(undefined);
